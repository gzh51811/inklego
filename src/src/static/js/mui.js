...
var mui = (function(document, undefined) {
	...
	$.trigger = function(element, eventType, eventData) {
		if (eventType=='scroll'){
			if (parseInt(eventData.y*-1) > header) {document.querySelector('#fixed').classList.remove('mui-hidden');}
			else {document.querySelector('#fixed').classList.add('mui-hidden');}
		}
...
(function($, window, document, undefined) {
	...
	var Scroll = $.Class.extend({
		...
		_drag: function(e) {
			...
			this.x = newX;
			this.y = newY;
			if (this.y*-1 >= header) {document.querySelector('#fixed').classList.remove('mui-hidden');}
			else {document.querySelector('#fixed').classList.add('mui-hidden');}
